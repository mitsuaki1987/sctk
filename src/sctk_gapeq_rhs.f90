...
SUBROUTINE gapeq_rhs(delta,res)
  ...
  REAL(dp) :: chi(ngapmax,2), dosh(2), Kh, dlth(2), xmax(2)
  !
  ...
  IF(zero_kelvin) THEN
    DO ii = 1, 2
      chi(1:ngap(ii),ii) = 1.0_dp
    END DO
  ELSE
    DO ii = 1, 2
      chi(1:ngap(ii),ii) = TANH(0.5_dp * beta * SQRT(xi(1:ngap(ii),ii)**2 + delta(1:ngap(ii),ii)**2))
    END DO
  END IF
  !
  DO ii = 1, 2
