!
! Copyright (C) 2017 Mitsuaki Kawamura
! This file is distributed under the terms of the
! GNU General Public License. See the file `License'
! in the root directory of the present distribution,
! or http://www.gnu.org/copyleft/gpl.txt .
!
MODULE sctk_ini_delta
  !
  IMPLICIT NONE
  !
CONTAINS
!
! Define initial delta
!
SUBROUTINE ini_delta(lallocate)
  !
  USE kinds, ONLY : DP
  USE mp_world, ONLY : world_comm
  USE mp, ONLY : mp_bcast, mp_max
  USE io_global, ONLY : stdout, ionode, ionode_id
  USE modes, ONLY : nmodes
  USE disp,  ONLY : nqs
  USE io_files, ONLY : prefix, tmp_dir
  USE input_parameters, ONLY : restart_mode
  USE sctk_val, ONLY : bindx, delta, dk, dx0, kindx, emin, initial_delta, dxq, &
  &                     ngapmax, ngap, nx, omg0, xi, xi0
  !
  USE sctk_cnt_dsp, ONLY : cnt_and_dsp
  !
  IMPLICIT NONE
  !
  LOGICAL,INTENT(IN) :: lallocate
  !
  INTEGER :: it, fi = 10, ios, cnt, dsp, ii
  REAL(dp) :: thr, Z0, dosf
  CHARACTER(1) :: tmp
  !
  IF(TRIM(restart_mode) == "restart") THEN
     !
     WRITE(stdout,'(7x,a)') &
     &  "Initial delta is read from file (" // TRIM(tmp_dir) // TRIM(prefix) // "_delta.dat)"
     !
     IF(ionode) THEN
        !
        OPEN(fi, file = TRIM(tmp_dir) // TRIM(prefix) // '.scgap', &
        &    status="old", action = 'read', iostat = ios)
        IF(ios /= 0) CALL errore("ini_delta", &
        &                        "Can not open" // TRIM(tmp_dir) // TRIM(prefix) // ".scgap", 1)
        !
        READ(fi,*) tmp, ngap(1:2)
        !
        ngapmax = MAXVAL(ngap(1:2))
        WRITE(*,'(7x,"Number of total points for gap equation : ",2(i0,2x))') ngap(1:2)
        IF(lallocate) &
        &  ALLOCATE(xi(ngapmax,2), delta(ngapmax,2),    dk(ngapmax,2), &
        &          dxq(ngapmax,2), kindx(ngapmax,2), bindx(ngapmax,2)  )
        xi(   1:ngapmax,1:2) = 0.0_dp
        delta(1:ngapmax,1:2) = 0.0_dp
        dk(   1:ngapmax,1:2) = 0.0_dp
        dxq(  1:ngapmax,1:2) = 0.0_dp
        kindx(1:ngapmax,1:2) = 0
        bindx(1:ngapmax,1:2) = 0
        !
        DO ii = 1, 2
          DO it = 1, ngap(ii)
            READ(fi,*) xi(it,ii), delta(it,ii), Z0, dk(it,ii), &
            &         dxq(it,ii), kindx(it,ii), bindx(it,ii)
          END DO
        END DO
        !
        CLOSE(fi)
        !
     END IF ! (ionode)
     !
     CALL mp_bcast(ngapmax,  ionode_id, world_comm )
     CALL mp_bcast(ngap, ionode_id, world_comm )
     IF((.NOT. ionode) .AND. lallocate) &
     &  ALLOCATE(xi(ngapmax,2), delta(ngapmax,2), dk(ngapmax,2), &
     &                    kindx(ngapmax,2), bindx(ngapmax,2))
     CALL mp_bcast(xi,    ionode_id, world_comm )
     CALL mp_bcast(delta, ionode_id, world_comm )
     CALL mp_bcast(dk,    ionode_id, world_comm )
     CALL mp_bcast(kindx, ionode_id, world_comm )
     CALL mp_bcast(bindx, ionode_id, world_comm )
     !
     DO ii = 1, 2
       dosf = SUM(dk(1:ngap(ii),ii), ABS(xi(1:ngap(ii),ii)) < emin)
       dosf = dosf / dx0(minloc(ABS(xi0(1:nx)), 1))
       WRITE(stdout,'(7x,"DOS(E_F)[Ry^-1/cell/spin] : ",e12.5)') dosf
     END DO
     !
     IF(MAXVAL(ABS(delta(1:ngap(1),1))) < 1.0e-8_dp) &
     &  WRITE(stdout,'(7x,"!!!CAUTION!!! Delta from file is too small.")')
     !
  ELSE
     !
     WRITE(stdout,'(7x,"Initial delta is theta function")')
     !
     CALL compute_d3k()
     !
     ALLOCATE(delta(ngapmax,2))
     !
     CALL cnt_and_dsp(nqs,cnt,dsp)
     thr = MAXVAL(omg0(1:nmodes,1:cnt))
     CALL mp_max(thr, world_comm)
     !
     IF(ionode) THEN
        CALL random_seed()
        CALL random_number(delta(1:ngapmax,1:2))
        IF(initial_delta == "random-nodeless") THEN
           delta(1:ngapmax,1:2) = 1.0_dp * (delta(1:ngapmax,1:2) - 0.0_dp) * thr
        ELSE IF(initial_delta == "random-nodal") THEN
           delta(1:ngapmax,1:2) = 2.0_dp * (delta(1:ngapmax,1:2) - 0.5_dp) * thr
        ELSE IF(initial_delta == "constant") THEN
           delta(1:ngapmax,1:2) = thr
        ELSE IF(initial_delta == "band-constant") THEN
          DO ii = 1, 2
            DO it = 1, ngap(ii)
              IF(MOD(bindx(it,ii), 2) == 0) THEN
                delta(it, ii) = -thr
              ELSE
                delta(it, ii) = thr
              END IF
            END DO
          END DO
        ELSE
           CALL errore ('ini_delta', 'Invalid initial_delta in input', 1)
        END IF
     END IF
     CALL mp_bcast(delta,    ionode_id, world_comm )
     !
     !delta(1:ngapmax,1:2) = thr**2 / (thr + xi(1:ngapmax,1:2))
     !
  END IF
  !
END SUBROUTINE ini_delta
!
! Compute energy groid
!
SUBROUTINE energy_grid()
  !
  USE kinds, ONLY : DP
  USE klist, ONLY : nks
  USE el_phon, ONLY : elph_nbnd_min, elph_nbnd_max
  USE wvfct, ONLY : et
  USE ener, ONLY : ef
  USE sctk_val, ONLY : dx0, emin, nx, xi0, Zemin
  !
  USE sctk_gauss_legendre, ONLY : weightspoints_gl
  !
  IMPLICIT NONE
  !
  INTEGER :: ix, imin
  REAL(dp) :: xmax, xmin, xx, ww, rhs(nx), rnx
  !
  ! Make logscale
  !
  xmax = MAXVAL(et(elph_nbnd_min:elph_nbnd_max,1:nks) - ef)
  xmin = MINVAL(et(elph_nbnd_min:elph_nbnd_max,1:nks) - ef)
  !
  ALLOCATE(xi0(nx), dx0(nx))
  CALL weightspoints_gl(nx,xi0,dx0)
  !
  rnx = REAL(nx, dp)
  !
  DO ix = 1, nx
     rhs(ix) = (1.0_dp + xi0(ix) - 2.0_dp / rnx) &
     &         * LOG(  xmax / (0.5_dp * rnx * emin * (1.0_dp - xi0(ix)))) &
     &       - (1.0_dp - xi0(ix) - 2.0_dp / rnx) &
     &         * LOG(- xmin / (0.5_dp * rnx * emin * (1.0_dp + xi0(ix))))
     rhs(ix) = ABS(rhs(ix))
  END DO
  !
  imin = MINLOC(rhs(1:nx),1)
  xx = xi0(imin)
  ww = MAX(LOG(- xmin / (0.5_dp * rnx * emin * (1.0_dp + xx))) / (1.0_dp + xx - 2.0_dp / rnx), &
  &        LOG(  xmax / (0.5_dp * rnx * emin * (1.0_dp - xx))) / (1.0_dp - xx - 2.0_dp / rnx)  )
  !
  DO ix = 1, nx
     dx0(ix) = dx0(ix) * (1.0_dp + ww * ABS(xi0(ix) - xx)) &
     &                        * emin * 0.5_dp * rnx * exp(ww * (ABS(xi0(ix) - xx) - 2.0_dp / rnx)) 
     xi0(ix) = (xi0(ix) - xx) * emin * 0.5_dp * rnx * exp(ww * (ABS(xi0(ix) - xx) - 2.0_dp / rnx))
  END DO
  emin = ABS(dx0(imin+1)) * 0.5_dp
  Zemin = ABS(dx0(imin)) * 0.5_dp
  !
END SUBROUTINE energy_grid
!
! Cp,@ite DOS_k
!
SUBROUTINE compute_dosk(dos, dxq_dos, d2xq_dos)
  !
  USE kinds, ONLY : DP
  USE mp_world, ONLY : world_comm
  USE mp, ONLY : mp_sum
  USE start_k, ONLY : nk1, nk2, nk3
  USE disp,  ONLY : nq1, nq2, nq3
  USE klist, ONLY : nks
  USE el_phon, ONLY : elph_nbnd_min, elph_nbnd_max
  USE elph_tetra_mod, ONLY : lshift_q
  USE sctk_val, ONLY : nqbz, nx, d_et, d2_et
  !
  USE sctk_tetra, ONLY : calc_dosk, interpol_indx
  IMPLICIT NONE
  !
  REAL(dp),INTENT(OUT) :: dos(nx*(elph_nbnd_max-elph_nbnd_min+1),nqbz,2), &
  &                   dxq_dos(nx*(elph_nbnd_max-elph_nbnd_min+1),nqbz,2), &
  &                  d2xq_dos(nx*(elph_nbnd_max-elph_nbnd_min+1),nqbz,2)
  !
  INTEGER :: ik, ikv(3), kintp(8), i8, ibnd, ii
  REAL(dp) :: kv(3), wintp(8), dosd(nx*(elph_nbnd_max-elph_nbnd_min+1),nks)
  REAL(dp),ALLOCATABLE :: dxq_dosd(:), d2xq_dosd(:)
  !
  ! Average and Differential energy for finite-q SCDFT
  !
  CALL calc_d_xi_q()
  !
  ! k-dependent weight for delta(xi-xi_{n k})
  !
  CALL calc_dosk(dosd)
  !
  ! Interpolation of weight
  !
  dos(     1:nx*(elph_nbnd_max-elph_nbnd_min+1),1:nqbz,1:2) = 0.0_dp
  dxq_dos( 1:nx*(elph_nbnd_max-elph_nbnd_min+1),1:nqbz,1:2) = 0.0_dp
  d2xq_dos(1:nx*(elph_nbnd_max-elph_nbnd_min+1),1:nqbz,1:2) = 0.0_dp
  !
  !!$OMP PARALLEL DEFAULT(NONE) &
  !!$OMP & SHARED(nks,nqbz,nx,dosd,dos,dxq_dos,d2xq_dos,elph_nbnd_min,elph_nbnd_max,nk1, &
  !!$OMP &        nk2,nk3,nq1,nq2,nq3,d_et,d2_et,world_comm) &
  !!$OMP & PRIVATE(ik,ikv,kv,wintp,kintp,i8,dxq_dosd,d2xq_dosd,ii)
  !
  ALLOCATE(dxq_dosd(nx*(elph_nbnd_max-elph_nbnd_min+1)), &
  &       d2xq_dosd(nx*(elph_nbnd_max-elph_nbnd_min+1))  )
  !
  DO ii = 1, 2
    !
    !!$OMP DO REDUCTION(+: dos, dxq_dos, d2xq_dos)
    DO ik = 1, nks
      !
      ikv(1) = (ik - 1) / (nk2*nk3)
      ikv(2) = (ik - 1 - ikv(1)*nk2*nk3) / nk3
      ikv(3) =  ik - 1 - ikv(1)*nk2*nk3 - ikv(2)*nk3
      kv(1:3) = REAL(ikv(1:3), dp) / REAL((/nk1,nk2,nk3/), dp) &
      &       - 0.5_dp * REAL((ii-1)*lshift_q(1:3),DP) / REAL((/nq1,nq2,nq3/), dp)
      CALL interpol_indx((/nq1,nq2,nq3/),kv,kintp,wintp)
      DO i8 = 1, 8
        dos(     1:nx*(elph_nbnd_max-elph_nbnd_min+1),      kintp(i8),ii) &
        & = dos( 1:nx*(elph_nbnd_max-elph_nbnd_min+1),      kintp(i8),ii) &
        & + dosd(1:nx*(elph_nbnd_max-elph_nbnd_min+1),ik) * wintp(i8)
      END DO
      !
      DO ibnd = 1, elph_nbnd_max-elph_nbnd_min+1
        dxq_dosd( (ibnd-1)*nx+1:ibnd*nx) = dosd((ibnd-1)*nx+1:ibnd*nx,ik) * d_et( elph_nbnd_min-1+ibnd,ik)
        d2xq_dosd((ibnd-1)*nx+1:ibnd*nx) = dosd((ibnd-1)*nx+1:ibnd*nx,ik) * d2_et(elph_nbnd_min-1+ibnd,ik)
      END DO
      !
      DO i8 = 1, 8
        dxq_dos(      1:nx*(elph_nbnd_max-elph_nbnd_min+1),   kintp(i8),ii) &
        & = dxq_dos(  1:nx*(elph_nbnd_max-elph_nbnd_min+1),   kintp(i8),ii) &
        &  + dxq_dosd(1:nx*(elph_nbnd_max-elph_nbnd_min+1)) * wintp(i8)
        !
        d2xq_dos(      1:nx*(elph_nbnd_max-elph_nbnd_min+1),   kintp(i8),ii) &
        & = d2xq_dos(  1:nx*(elph_nbnd_max-elph_nbnd_min+1),   kintp(i8),ii) &
        &  + d2xq_dosd(1:nx*(elph_nbnd_max-elph_nbnd_min+1)) * wintp(i8)
      END DO
      !
    END DO
    !!$OMP END DO
    !
  END DO ! ii
  !
  DEALLOCATE(dxq_dosd, d2xq_dosd)
  !
  !!$OMP END PARALLEL
  !
  CALL mp_sum(dos,     world_comm)
  CALL mp_sum(dxq_dos, world_comm)
  CALL mp_sum(d2xq_dos, world_comm)
  !
  CALL symm_dosk(dos, dxq_dos, d2xq_dos)
  !
  dxq_dos(:,:,:) = dxq_dos(:,:,:) / dos(:,:,:)
  d2xq_dos(:,:,:) = d2xq_dos(:,:,:) / dos(:,:,:)
  !
END SUBROUTINE compute_dosk
!
! Average and Differential energy for finite-q SCDFT
!
SUBROUTINE calc_d_xi_q()
  !
  USE kinds, ONLY : dp
  USE wvfct, ONLY : nbnd, et
  USE klist, ONLY : nks
  USE start_k, ONLY : nk1, nk2, nk3
  USE sctk_val, ONLY : d_et, q_fflo, d2_et
  !
  INTEGER :: i1, i2, i3, ii, ikv(3), ikvp(3,3), ikvm(3,3), ikvpp(3,3), ikvmm(3,3), &
  &          ik, ikp(3), ikm(3), ikpp(3), ikmm(3), iip, iim, ibnd
  REAL(dp) :: rnk(3), cmat(3,3), cq(3), qCq
  REAL(dp),ALLOCATABLE :: et0(:,:)
  !
  rnk(1:3) = REAL((/nk1, nk2, nk3/), dp)
  !
  ALLOCATE(d_et(nbnd,nks), et0(nbnd,nks), d2_et(nbnd,nks))
  et0(1:nbnd, 1:nks) = et(1:nbnd, 1:nks)
  !
  !$OMP PARALLEL DEFAULT(none) &
  !$OMP & SHARED(nk1,nk2,nk3,nbnd,et,d_et,d2_et,et0,rnk,q_fflo) &
  !$OMP & PRIVATE(i1,i2,i3,ikv,ikvp,ikvm,ikvpp,ikvmm,ik,ikp,ikm,ikpp,ikmm,ii,iip,iim,ibnd,cmat,cq, qcq)
  !
  !$OMP DO
  DO i1 = 1, nk1
     DO i2 = 1, nk2
        DO i3 = 1, nk3
           !
           ikv(1:3) = (/i1, i2, i3/) - 1
           !
           ! k +- b_1/n_1, k +- b_2/n_2, k +- b_3/n_3
           ! k +- (b_2/n_2 + b_3/n_3), etc.
           !
           DO ii = 1, 3
              ikvp(1:3, ii) = ikv(1:3)
              ikvm(1:3, ii) = ikv(1:3)
              ikvpp(1:3, ii) = ikv(1:3) + 1
              ikvmm(1:3, ii) = ikv(1:3) - 1
           END DO
           !
           DO ii = 1, 3
              ikvp(ii, ii) = ikvp(ii, ii) + 1
              ikvm(ii, ii) = ikvm(ii, ii) - 1
              ikvpp(ii, ii) = ikvpp(ii, ii) - 1
              ikvmm(ii, ii) = ikvmm(ii, ii) + 1
           END DO
           !
           ! Integer k-vector -> one-dimensional k index
           !
           ikv  = MODULO(ikv,  (/nk1, nk2, nk3/))
           ik  = 1 + ikv(3) + nk3 * ikv(2) + nk3 * nk2 * ikv(1)
           DO ii = 1, 3
              ikvp(1:3,ii) = MODULO(ikvp(1:3,ii), (/nk1, nk2, nk3/))
              ikvm(1:3,ii) = MODULO(ikvm(1:3,ii), (/nk1, nk2, nk3/))
              ikvpp(1:3,ii) = MODULO(ikvpp(1:3,ii), (/nk1, nk2, nk3/))
              ikvmm(1:3,ii) = MODULO(ikvmm(1:3,ii), (/nk1, nk2, nk3/))
              !
              ikp(ii) = 1 + ikvp(3,ii) + nk3 * ikvp(2,ii) + nk3 * nk2 * ikvp(1,ii)
              ikm(ii) = 1 + ikvm(3,ii) + nk3 * ikvm(2,ii) + nk3 * nk2 * ikvm(1,ii)
              ikpp(ii) = 1 + ikvpp(3,ii) + nk3 * ikvpp(2,ii) + nk3 * nk2 * ikvpp(1,ii)
              ikmm(ii) = 1 + ikvmm(3,ii) + nk3 * ikvmm(2,ii) + nk3 * nk2 * ikvmm(1,ii)
           END DO
           !
           DO ibnd = 1, nbnd
              !
              ! (xi_{k+q/2} - xi_{k-q/2}) / 2
              !
              d_et(ibnd, ik) = 0.0_dp
              DO ii = 1, 3
                 d_et(ibnd, ik) = d_et(ibnd, ik) &
                 &              + rnk(ii) * 0.5_dp * (et0(ibnd, ikp(ii)) - et0(ibnd, ikm(ii))) * 0.5_dp * q_fflo(ii)
              END DO
              !
              ! (xi_{k+q/2} + xi_{k-q/2}) / 2
              ! Overwrite module variable "et"
              !
              DO ii = 1, 3
                 cmat(ii,ii) = 2.0_dp * rnk(ii)**2 * (0.5_dp * (et0(ibnd, ikp(ii)) + et0(ibnd, ikm(ii))) - et0(ibnd, ik))
                 iip = MODULO(ii + 1 - 1, 3) + 1
                 iim = MODULO(ii - 1 - 1, 3) + 1
                 cmat(iip,iim) = rnk(iim) * rnk(iip) * (0.5_dp * (et0(ibnd, ikpp(ii)) + et0(ibnd, ikmm(ii))) &
                 &                                     -0.5_dp * (et0(ibnd, ikp(iip)) + et0(ibnd, ikm(iip))) &
                 &                                     -0.5_dp * (et0(ibnd, ikp(iim)) + et0(ibnd, ikm(iim))) &
                 &                                     + et0(ibnd, ik))
                 cmat(iim,iip) = cmat(iip,iim)
              END DO
              !
              cq(1:3) = MATMUL(cmat(1:3, 1:3), 0.5_dp * q_fflo(1:3))
              qCq = 0.5_dp * DOT_PRODUCT(0.5_dp * q_fflo, cq)
              et(ibnd, ik) = et0(ibnd, ik) + qCq
              d2_et(ibnd, ik) = qCq
              !
           END DO
           !
        END DO
     END DO
  END DO
  !$OMP END DO
  !$OMP END PARALLEL
  !
  DEALLOCATE(et0)
  !
END SUBROUTINE calc_d_xi_q
!
! Compute integration weights
!
SUBROUTINE compute_d3k()
  !
  USE kinds, ONLY : DP
  USE io_global, ONLY : stdout
  USE el_phon, ONLY : elph_nbnd_min, elph_nbnd_max
  USE elph_tetra_mod, ONLY : lshift_q
  USE sctk_val, ONLY : bindx, dk, fbee, kindx, lbee, dxq, d2xq, &
  &                     nqbz, ngapmax, ngap, nx, xi, xi0
  !
  IMPLICIT NONE
  !
  INTEGER :: ik, ib, ix, ii
  REAL(dp) :: dos(nx,elph_nbnd_min:elph_nbnd_max,nqbz,2), thr = 1e-12_dp, shift(3), &
  &       dxq_dos(nx,elph_nbnd_min:elph_nbnd_max,nqbz,2), &
  &      d2xq_dos(nx,elph_nbnd_min:elph_nbnd_max,nqbz,2)
  !
  CALL compute_dosk(dos, dxq_dos, d2xq_dos)
  !
  ix = MINLOC(ABS(xi0(1:nx)),1)
  DO ii = 1, 2
    WRITE(stdout,'(7x,"DOS(E_F)[/Ry/cell/spin] : ",e12.5)') SUM(dos(ix,elph_nbnd_min:elph_nbnd_max,1:nqbz,ii))
  END DO
  !
  ! Query of ngap(1:2)
  !
  DO ii = 1, 2
    !
    ngap(ii) = 0
    DO ik = 1, nqbz
      DO ib = fbee, lbee
        !
        IF(elph_nbnd_min <= ib .AND. ib <= elph_nbnd_max) THEN
           ngap(ii) = ngap(ii) + MAX(1, count(dos(1:nx,ib,ik,ii) > thr))
        ELSE
           ngap(ii) = ngap(ii) + 1
        END IF
        !
      END DO
    END DO
    !
  END DO ! ii = 1, 2
  !
  WRITE(stdout,'(7x,"Number of total points for gap equation : ",2(i0,2x))') ngap(1:2)
  ngapmax = MAXVAL(ngap(1:2))
  ALLOCATE(xi(ngapmax,2),  dk(ngapmax,2), kindx(ngapmax,2), &
  &     bindx(ngapmax,2), dxq(ngapmax,2),  d2xq(ngapmax,2)  )
  !
  xi(   1:ngapmax,1:2) = 0.0_dp
  dk(   1:ngapmax,1:2) = 0.0_dp
  kindx(1:ngapmax,1:2) = 0
  bindx(1:ngapmax,1:2) = 0
  dxq(  1:ngapmax,1:2) = 0.0_dp
  d2xq( 1:ngapmax,1:2) = 0.0_dp
  !
  ! Map xi, d3k, dosk, indx, bindx, dxq
  !
  DO ii = 1, 2
    !
    IF(ii == 1) THEN
      shift(1:3) = 0.0_dp
    ELSE
      shift(1:3) = 0.5_dp * REAL(lshift_q(1:3), DP)
    END IF
    !
    CALL store_d3k(dos(1:nx,elph_nbnd_min:elph_nbnd_max,1:nqbz,ii), &
    &          dxq_dos(1:nx,elph_nbnd_min:elph_nbnd_max,1:nqbz,ii), &
    &         d2xq_dos(1:nx,elph_nbnd_min:elph_nbnd_max,1:nqbz,ii), shift, ngap(ii), &
    &               xi(1:ngap(ii),ii),  dk(1:ngap(ii),ii), kindx(1:ngap(ii),ii), &
    &            bindx(1:ngap(ii),ii), dxq(1:ngap(ii),ii),  d2xq(1:ngap(ii),ii))
    !
  END DO
  !
END SUBROUTINE compute_d3k
!
! Map d3k, dosk, xi, kindx, bindx
!
SUBROUTINE store_d3k(dos,dxq_dos,d2xq_dos,shift,ngap,xi,dk,kindx,bindx,dxq,d2xq)
  !
  USE kinds, ONLY : DP
  USE start_k, ONLY : nk1, nk2, nk3
  USE disp,  ONLY : nq1, nq2, nq3
  USE wvfct, ONLY : et
  USE el_phon, ONLY : elph_nbnd_min, elph_nbnd_max
  USE ener, ONLY : ef
  !
  USE sctk_val, ONLY : dx0, fbee, lbee, nqbz, nx, xi0, d_et, d2_et
  USE sctk_tetra, ONLY : interpol_indx
  !
  IMPLICIT NONE
  !
  INTEGER,INTENT(IN) :: ngap
  REAL(dp),INTENT(IN) :: dos(nx,elph_nbnd_min:elph_nbnd_max,nqbz), &
  &                  dxq_dos(nx,elph_nbnd_min:elph_nbnd_max,nqbz), &
  &                 d2xq_dos(nx,elph_nbnd_min:elph_nbnd_max,nqbz), shift(3)
  REAL(dp),INTENT(OUT) :: xi(ngap), dk(ngap), dxq(ngap), d2xq(ngap)
  INTEGER,INTENT(OUT) :: kindx(ngap), bindx(ngap)
  !
  INTEGER :: ngap0, ik, ib, ix, kintp(8), ikv(3)
  REAL(dp) :: thr = 1e-12_dp, kv(3), wintp(8)
  !
  ngap0 = 0
  DO ik = 1, nqbz
     !
     ikv(1) = (ik - 1) / (nq2*nq3)
     ikv(2) = (ik - 1 - ikv(1)*nq2*nq3) / nq3
     ikv(3) =  ik - 1 - ikv(1)*nq2*nq3 - ikv(2)*nq3
     !
     DO ib = fbee, lbee
        !
        IF(elph_nbnd_min <= ib .AND. ib <= elph_nbnd_max) THEN
           !
           IF(COUNT(dos(1:nx,ib,ik) > thr) >= 2) THEN
              !
              DO ix = 1, nx                        
                 !
                 IF(dos(ix,ib,ik) > thr) THEN
                    !
                    ngap0 = ngap0 + 1
                    !
                    xi(ngap0) = xi0(ix)
                    dk(ngap0) = dos(ix,ib,ik) * dx0(ix)
                    dxq(ngap0) = dxq_dos(ix,ib,ik)
                    d2xq(ngap0) = d2xq_dos(ix,ib,ik)
                    kindx(ngap0) = ik
                    bindx(ngap0) = ib
                    !
                 END IF
                 !
              END DO ! ix
              !
              CYCLE
              !
           END IF
           !
        END IF
        !
        ! If this state is far from Fermi level
        !
        ngap0 = ngap0 + 1
        !
        kv(1:3) = (REAL(ikv(1:3), dp) + shift(1:3)) / REAL((/nq1,nq2,nq3/), dp)
        CALL interpol_indx((/nk1,nk2,nk3/),kv,kintp,wintp)
        xi(ngap0) = DOT_PRODUCT(wintp(1:8), et(ib,kintp(1:8))) - ef
        dxq(ngap0) = DOT_PRODUCT(wintp(1:8), d_et(ib,kintp(1:8)))
        d2xq(ngap0) = DOT_PRODUCT(wintp(1:8), d2_et(ib,kintp(1:8)))
        !
        dk(ngap0) = 1.0_dp / REAL(nqbz, dp)
        kindx(ngap0) = ik
        bindx(ngap0) = ib
        !
     END DO ! ib
     !
  END DO ! ik
  !
  IF(ngap0 /= ngap) THEN
     CALL errore ('store_d3k', 'ngap0 /= ngapmax', ngap)
  END IF
  !
END SUBROUTINE store_d3k
!
! Symmetrize Dos
!
SUBROUTINE symm_dosk(dos, dxq_dos,d2xq_dos)
  !
  USE kinds, ONLY : DP
  USE symm_base, ONLY : nsym, s
  USE disp,  ONLY : nq1, nq2, nq3
  USE el_phon, ONLY : elph_nbnd_min, elph_nbnd_max
  USE io_global, ONLY : stdout
  USE sctk_val, ONLY : nqbz, nx, q_fflo
  USE elph_tetra_mod, ONLY : lshift_q
  !
  IMPLICIT NONE
  !
  REAL(dp),INTENT(inout) :: dos(nx,elph_nbnd_min:elph_nbnd_max,nqbz,2), &
  &                     dxq_dos(nx,elph_nbnd_min:elph_nbnd_max,nqbz,2), &
  &                    d2xq_dos(nx,elph_nbnd_min:elph_nbnd_max,nqbz,2)
  !
  INTEGER :: ik, ik2, isym, ikv2(3), nksym(nqbz), ii
  REAL(dp) :: dostmp(nx,elph_nbnd_min:elph_nbnd_max,nqbz), kv1(3), kv2(3), q_fflo2(3), qnorm, &
  &       dxq_dostmp(nx,elph_nbnd_min:elph_nbnd_max,nqbz), d2xq_dostmp(nx,elph_nbnd_min:elph_nbnd_max,nqbz)
  LOGICAL :: lq_sym(nsym)
  !
  ! Only symmetries conserving q_fflo is used.
  !
  lq_sym(1:nsym) = .FALSE.
  DO isym = 1, nsym
     q_fflo2(1:3) = MATMUL(REAL(s(1:3,1:3,isym), dp), q_fflo(1:3))
     q_fflo2(1:3) = q_fflo2(1:3) - q_fflo(1:3)
     qnorm = DOT_PRODUCT(q_fflo2, q_fflo2)
     IF(qnorm < 1.0e-15_dp) lq_sym(isym) = .TRUE.
  END DO
  WRITE(stdout,'(7x,"Number of symmetries conserving q_FFLO : ",i5)') COUNT(lq_sym(1:nsym)) 
  !
  DO ii = 1, 2
    !
    nksym(1:nqbz) = 0
    dostmp(     1:nx,elph_nbnd_min:elph_nbnd_max,1:nqbz) = 0.0_dp
    dxq_dostmp( 1:nx,elph_nbnd_min:elph_nbnd_max,1:nqbz) = 0.0_dp
    d2xq_dostmp(1:nx,elph_nbnd_min:elph_nbnd_max,1:nqbz) = 0.0_dp
    !
    DO ik = 1, nqbz
      !
      ikv2(1) = (ik - 1) / (nq2*nq3)
      ikv2(2) = (ik - 1 - ikv2(1)*nq2*nq3) / nq3
      ikv2(3) =  ik - 1 - ikv2(1)*nq2*nq3 - ikv2(2)*nq3
      kv1(1:3) = (REAL(ikv2(1:3), dp) + 0.5_dp*REAL((ii-1)*lshift_q(1:3), DP)) / REAL((/nq1,nq2,nq3/), dp)
      !
      DO isym = 1, nsym
        !
        IF(.NOT. lq_sym(isym)) CYCLE
        !
        kv2(1:3) = MATMUL(REAL(s(1:3,1:3,isym), dp), kv1(1:3)) * REAL((/nq1,nq2,nq3/), dp)
        kv2(1:3) = kv2(1:3) - 0.5_dp * REAL((ii-1)*lshift_q(1:3),dp)
        ikv2(1:3) = NINT(kv2(1:3))
        !
        IF(ANY(ABS(kv2(1:3) - REAL(ikv2(1:3), dp)) > 1e-8_dp)) CYCLE
        !
        ikv2(1:3) = MODULO(ikv2(1:3), (/nq1,nq2,nq3/))
        ik2 = 1 + ikv2(3) + ikv2(2)*nq3 + ikv2(1)*nq2*nq3
        !
        nksym(ik2) = nksym(ik2) + 1
        !
        dostmp(    1:nx,elph_nbnd_min:elph_nbnd_max,ik2) &
        & = dostmp(1:nx,elph_nbnd_min:elph_nbnd_max,ik2) &
        & + dos(   1:nx,elph_nbnd_min:elph_nbnd_max,ik,ii)
        !
        dxq_dostmp(    1:nx,elph_nbnd_min:elph_nbnd_max,ik2) &
        & = dxq_dostmp(1:nx,elph_nbnd_min:elph_nbnd_max,ik2) &
        & + dxq_dos(   1:nx,elph_nbnd_min:elph_nbnd_max,ik,ii)
        !
        d2xq_dostmp(    1:nx,elph_nbnd_min:elph_nbnd_max,ik2) &
        & = d2xq_dostmp(1:nx,elph_nbnd_min:elph_nbnd_max,ik2) &
        & + d2xq_dos(   1:nx,elph_nbnd_min:elph_nbnd_max,ik,ii)
        !
      END DO
      !
    END DO
    !
    DO ik = 1, nqbz
      dos(       1:nx,elph_nbnd_min:elph_nbnd_max,ik,ii) &
      & = dostmp(1:nx,elph_nbnd_min:elph_nbnd_max,ik) / REAL(nksym(ik), dp)
      !
      dxq_dos(       1:nx,elph_nbnd_min:elph_nbnd_max,ik,ii) &
      & = dxq_dostmp(1:nx,elph_nbnd_min:elph_nbnd_max,ik) / REAL(nksym(ik), dp)
      !
      d2xq_dos(       1:nx,elph_nbnd_min:elph_nbnd_max,ik,ii) &
      & = d2xq_dostmp(1:nx,elph_nbnd_min:elph_nbnd_max,ik) / REAL(nksym(ik), dp)
    END DO
    !
  END DO ! ii
  !
END SUBROUTINE symm_dosk
!
! Initialization for lambda and mu
!
SUBROUTINE ini_lambda_mu()
  !
  USE kinds, ONLY : DP
  USE mp_world, ONLY : world_comm
  USE mp, ONLY : mp_sum
  USE io_global, ONLY : stdout
  USE start_k, ONLY : nk1, nk2, nk3
  USE klist, ONLY : nks
  USE disp,  ONLY : nq1, nq2, nq3
  USE el_phon, ONLY : elph_nbnd_min, elph_nbnd_max
  USE fermisurfer_common, ONLY : b_low, b_high
  USE elph_tetra_mod, ONLY : lshift_q
  USE sctk_val, ONLY : bindx, dk, kindx, nqbz, ngapmax, nx, xi0, dx0
  !
  USE sctk_tetra, ONLY : calc_dosk, interpol_indx
  !
  IMPLICIT NONE
  INTEGER :: ik, ikv(3), ib, kintp(8), i8
  REAL(dp) :: dos(elph_nbnd_min:elph_nbnd_max,nqbz,2), &
  &          dosd(elph_nbnd_min:elph_nbnd_max,nks), kv(3), wintp(8)
  !
  nx = 1
  DEALLOCATE(xi0, dx0)
  ALLOCATE(xi0(nx), dx0(nx))
  xi0(1) = 0d0
  dx0(1) = 1d0
  !
  CALL calc_dosk(dosd)
  !
  ! Interpolation of weight
  !
  dos(elph_nbnd_min:elph_nbnd_max,1:nqbz,1:2) = 0.0_dp
  !
  !$OMP PARALLEL DEFAULT(NONE) &
  !$OMP & SHARED(nqbz,nx,dosd,dos,elph_nbnd_min,elph_nbnd_max, &
  !$OMP &        nk1,nk2,nk3,nq1,nq2,nq3,nks,lshift_q) &
  !$OMP & PRIVATE(ik,ikv,kv,wintp,kintp,i8)
  !
  !$OMP DO REDUCTION(+: dos)
  DO ik = 1, nks
     !
     ikv(1) = (ik - 1) / (nk2*nk3)
     ikv(2) = (ik - 1 - ikv(1)*nk2*nk3) / nk3
     ikv(3) =  ik - 1 - ikv(1)*nk2*nk3 - ikv(2)*nk3
     kv(1:3) = REAL(ikv(1:3), dp) / REAL((/nk1,nk2,nk3/), dp) - 0.5_dp*REAL(lshift_q(1:3), DP) / REAL((/nq1,nq2,nq3/), dp)
     CALL interpol_indx((/nq1,nq2,nq3/),kv,kintp,wintp)
     DO i8 = 1, 8
        dos(     elph_nbnd_min:elph_nbnd_max,      kintp(i8),2) &
        & = dos( elph_nbnd_min:elph_nbnd_max,      kintp(i8),2) &
        & + dosd(elph_nbnd_min:elph_nbnd_max,ik) * wintp(i8)
     END DO
     !
  END DO
  !$OMP END DO
  !$OMP END PARALLEL
  !
  CALL mp_sum( dos, world_comm )
  !
  CALL symm_dosk(dos, dos, dos)
  !
  WRITE(stdout,'(7x,"DOS(E_F)[/Ry/cell/spin] : ",e12.5)') SUM(dos(elph_nbnd_min:elph_nbnd_max,1:nqbz,2)) 
  !
  ngapmax = nqbz * (b_high-b_low+1)
  WRITE(stdout,'(7x,"Number of total points for gap equation : ",i0)') ngapmax
  ALLOCATE(dk(ngapmax,1), kindx(ngapmax,1), bindx(ngapmax,1))
  !
  ngapmax = 0
  DO ik = 1, nqbz
     DO ib = b_low, b_high
        ngapmax = ngapmax + 1
        dk(ngapmax,1) = dos(ib,ik,2)
        kindx(ngapmax,1) = ik
        bindx(ngapmax,1) = ib
     END DO
  END DO
  !
END SUBROUTINE ini_lambda_mu
!
END MODULE sctk_ini_delta
